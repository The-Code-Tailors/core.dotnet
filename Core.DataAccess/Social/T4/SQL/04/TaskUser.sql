...
BEGIN
	...
	FROM [TaskUser] WITH (READUNCOMMITTED) WHERE [VersioningId] = @Id ORDER BY [Id] DESC
END

GO


CREATE PROCEDURE [TaskUserSelectVersionHistoryItem]
(
	@Id BIGINT,
	@ItemId BIGINT
)
...
BEGIN
	...
	, (SELECT COUNT(*) FROM [Remark] WHERE [MasterGuid] = [TaskUser].[Guid] AND [DeleteDate] IS NULL AND [VersioningId] IS NULL) [SocialCount3]
	, (SELECT COUNT(*) FROM [Task] WHERE [MasterGuid] = [TaskUser].[Guid] AND [DeleteDate] IS NULL AND [VersioningId] IS NULL) [SocialCount4]
	FROM [TaskUser] WITH (READUNCOMMITTED) WHERE [VersioningId] = @Id AND [Id] = @ItemId
END
